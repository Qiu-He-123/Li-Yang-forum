...
// 事件触发：停留帖子详情超过后台阈值时，让宠物 AI 说一句话
usePetAiTracker({
  type: 'post_detail',
...
/** 求助帖：是否求助帖（有悬赏金币） */
const isHelpPost = computed(() => post.value?.reward_coins != null)
/** 求助帖：标记已解决/未解决 loading */
const solvedLoading = ref(false)

/** 求助帖楼主：标记已解决 / 取消解决 */
async function onToggleSolved() {
  if (!post.value || !isHelpPost.value || !isAuthor.value) return
  const nextSolved = !post.value.is_solved
  if (nextSolved) {
    // 标记解决需二次确认，避免误操作
    if (!confirm('确认将此求助帖标记为「已解决」？')) return
  }
  solvedLoading.value = true
  try {
    const { data } = await markPostSolved(post.value.id, nextSolved)
    post.value.is_solved = data.data.is_solved
    post.value.solved_at = data.data.solved_at
    toast.success(nextSolved ? '已标记为已解决' : '已恢复为未解决')
  } catch (err) {
    toast.error((err as Error).message)
  } finally {
  ...
  <main class="page-detail">
            ...
            <article class="post-card">
              <header class="author-row">
                ...
                <div class="author-meta">
                  ...
                  <!-- 发帖人的可互动宠物：直接放在作者信息框内（与首页/广场一致），
                       体积与漂浮宠一致(120px)、可拖动、随机小动作、可飞行；自己的帖子默认隐藏 -->
                  <PostPetBox
                    v-if="!post.is_anonymous && post.author_id && !isAuthor"
                    :user-id="post.author_id"
                    :size="120"
                    :self="false"
                    :author-name="post.author"
                    class="post-pet-detail"
                  />
              ...
              <!-- 帖子内容板块（标题/图片/正文/投票）：宠物悬浮在此"上方板块"内（不在发起人行） -->
              <div class="post-content-wrap">
              <h1 v-if="post.title" class="post-title">{{ post.title }}</h1>

              <!-- 求助帖：悬赏金币 + 解决状态横幅 -->
              <div v-if="isHelpPost" class="post-reward-banner" :class="post.is_solved ? 'is-solved' : 'is-open'">
                <div class="reward-left">
                  <Icon :name="post.is_solved ? 'check-circle' : 'circle-question'" :size="18" :color="post.is_solved ? '#1a9e5a' : '#e8890c'" />
                  <span class="reward-status">{{ post.is_solved ? '已解决' : '求助中' }}</span>
                </div>
                <div class="reward-coins">
                  <Icon name="coin" :size="15" color="#ffb300" />
                  <span class="reward-amount">{{ post.reward_coins }} 金币悬赏</span>
                </div>
                <button
                  v-if="isAuthor"
                  class="reward-toggle-btn"
                  type="button"
                  :disabled="solvedLoading"
                  @click="onToggleSolved"
                >
                  {{ post.is_solved ? '恢复未解决' : '标记已解决' }}
                </button>
              </div>
    ...
    <NativeDialog v-model="editDialogVisible" title="编辑帖子" width="640px">
      <PostEditor
        ...
        :initial-reward-coins="post.reward_coins"
...
.post-reward-banner.is-open {
  background: rgba(232, 137, 12, 0.08);
  border-color: rgba(232, 137, 12, 0.28);
}
.post-reward-banner.is-solved {
  background: rgba(26, 158, 90, 0.08);
  border-color: rgba(26, 158, 90, 0.28);
}
.reward-left {
  display: inline-flex;
  align-items: center;
...
.post-reward-banner.is-open .reward-status {
  color: #e8890c;
}
.post-reward-banner.is-solved .reward-status {
  color: #1a9e5a;
}
.reward-coins {
  display: inline-flex;
...
.post-reward-banner.is-solved .reward-toggle-btn {
  background: #1a9e5a;
}
.reward-toggle-btn:hover:not(:disabled) {
  opacity: 0.9;
}
.reward-toggle-btn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
