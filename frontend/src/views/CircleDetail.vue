          ...
          >
            <div class="post-main">
              <div class="post-author-row">
                ...
                <!-- 求助帖：解决状态（右侧） -->
                <span v-if="post.reward_coins != null" class="solved-badge" :class="post.is_solved ? 'is-solved' : 'is-open'">
                  <Icon :name="post.is_solved ? 'check-circle' : 'circle-question'" :size="11" />
                  {{ post.is_solved ? '已解决' : '未解决' }}
                </span>
                <!-- 吧主删除按钮 -->
                <button
                  v-if="isAdmin"
...
.solved-badge.is-solved {
  color: #1a9e5a;
  background: color-mix(in srgb, #1a9e5a 14%, var(--bg-50));
}
.solved-badge.is-open {
  color: #e8890c;
  background: color-mix(in srgb, #e8890c 14%, var(--bg-50));
}
