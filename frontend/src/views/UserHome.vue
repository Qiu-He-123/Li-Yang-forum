...
const activeTab = ref<'posts' | 'favorites' | 'likes' | 'badges' | 'pets'>('posts')
...
const tabs = computed(() => {
  const list: { key: typeof activeTab.value; label: string; count: number }[] = []
  // 宠物 Tab：仅自己主页展示（第一位）
  if (isMe.value) list.push({ key: 'pets', label: '宠物', count: myPets.value.length })
  list.push(
    { key: 'posts', label: '作品', count: profile.value?.post_count ?? 0 },
    { key: 'favorites', label: '收藏', count: 0 },
    { key: 'likes', label: '点赞', count: 0 },
    { key: 'badges', label: '勋章', count: profile.value?.badge_count ?? 0 },
  )
  return list
})

// ============ 我的宠物（自己主页宠物 Tab） ============
const myPets = ref<MyPetItem[]>([])
const myPetsLoading = ref(false)

async function loadMyPets() {
  if (!isMe.value || !session.isLoggedIn()) return
  myPetsLoading.value = true
  try {
    const { data: resp } = await listMyPets({ showGlobalLoading: false, showGlobalError: false })
    myPets.value = resp.data.items ?? []
  } catch {
    myPets.value = []
  } finally {
...
function fmtPetDate(iso: string | null): string {
  if (!iso) return ''
...
function onTabChange(tab: 'posts' | 'favorites' | 'likes' | 'badges' | 'pets') {
  activeTab.value = tab
  if (tab === 'badges') {
    loadUserBadges()
    return
  ...
  if (tab === 'pets') {
    // 每次切到宠物 Tab 都拉最新（含饱食度/好感/等级变化）
    if (!myPetsLoading.value) loadMyPets()
    return
  }
  loadPosts()
}
...
function onFuncClick(item: (typeof funcGrid.value)[number]) {
  ...
  async function onShareProfile() {
    ...
    const title = `${displayName.value}的主页 - 同伴圈`
...
onActivated(async () => {
  ...
  // 宠物 Tab 一并刷新（keep-alive 返回时不走 onMounted，需主动重载，否则首次为空靠二次点击才出现）
  if (isMe.value && activeTab.value === 'pets') loadMyPets()
  loadInviteInfo()
...
onMounted(async () => {
  ...
  // 自己的主页默认落在「宠物」Tab，他人主页默认「作品」
  if (isMe.value) {
    activeTab.value = 'pets'
    await Promise.all([loadProfile(), loadMyPets(), loadWarningStatus(), minDelay])
  } else {
  ...
  <main v-else class="page-me">
    ...
    >
      <div class="hero-topbar">
        ...
        <span class="hero-title">{{ isMe ? '同伴圈·我的' : '个人主页' }}</span>
    ...
    <section class="posts-section">
      ...
      <!-- 宠物 Tab：仅自己主页，展示已领养宠物（点击进详情） -->
      <template v-if="activeTab === 'pets'">
        <div v-if="myPetsLoading" class="loading-tip">
          <Icon name="refresh" :size="20" />
          <span>加载中…</span>
        </div>

        <div v-else-if="myPets.length" class="pets-grid">
          <article
            v-for="pet in myPets"
            :key="pet.id"
            class="pet-card"
            @click="router.push(`/pet-shop/${pet.id}`)"
          >
            <div class="pet-card__stage">
              <PetAnimation v-if="pet.anim" :anim="pet.anim" :size="112" :interactive="false" />
              <img v-else-if="pet.image_url" class="pet-card__photo" :src="pet.image_url" :alt="pet.name" />
              <span v-else class="pet-card__emoji">{{ pet.image || '🐾' }}</span>
            </div>
            <div class="pet-card__info">
              <h4 class="pet-card__name">{{ pet.name }}</h4>
              <p class="pet-card__meta">
                <template v-if="pet.anim">{{ pet.anim.actions.map((a) => a.label).join(' · ') }}</template>
                <template v-else>{{ pet.category }}</template>
              </p>
              <p class="pet-card__date">🐾 {{ fmtPetDate(pet.adopted_at) }} 领养</p>
            </div>
          </article>
        </div>

        <!-- 空态：引导去商城 -->
        <div v-else class="pets-empty">
          <span class="pets-empty__emoji">🏠</span>
          <p class="pets-empty__text">宠物窝还空着，去领养一只像素宠物吧～</p>
          <button class="pets-empty__btn" type="button" @click="router.push('/pet-shop')">去商城领养</button>
        </div>
      </template>
...
.pet-card__stage::after {
  ...
  background: radial-gradient(ellipse at center, rgba(23, 32, 64, 0.12) 0%, transparent 68%);
  pointer-events: none;
}
.pet-card__photo {
  position: relative;
...
.pet-card__emoji { font-size: 52px; }
.pet-card__info { padding: 8px 12px 12px; }
.pet-card__name {
  margin: 0 0 3px;
  font-size: 14px;
  font-weight: 700;
...
.pets-empty__emoji { font-size: 48px; line-height: 1; }
.pets-empty__text {
  margin: 0;
  font-size: 14px;
  color: var(--text-500, #505464);
...
.pets-empty__btn {
  ...
  font-weight: 600;
  cursor: pointer;
  box-shadow: 0 4px 14px rgba(61, 123, 255, 0.32);
}
@media (min-width: 540px) {
  .pets-grid { grid-template-columns: repeat(3, 1fr); gap: 14px; }
}
