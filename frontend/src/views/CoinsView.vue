...
const missions = ref<DailyMissionItem[]>([])
...
async function loadAll() {
  try {
    ...
    shop.value = s.items
    coins.value = s.coins
    const m = (await getDailyMissions()).data.data
...
async function claim(m: DailyMissionItem) {
  if (claimingKey.value || !m.done || m.claimed) return
  claimingKey.value = m.key
  try {
    const data = (await claimDailyMission(m.key)).data.data
    coins.value = data.coins
    if (data.awarded > 0) toast.success(data.message)
    else toast.info(data.message)
    await loadAll()
  } catch (e: unknown) {
    const err = e as { response?: { data?: { msg?: string } } }
    toast.error(err.response?.data?.msg || '领取失败')
  } finally {
    claimingKey.value = null
  }
}

/** 任务未完成：点击按钮自动跳转到对应页面去完成任务 */
function goToMissionPage(key: string) {
  ...
  <div class="coins-page">
    ...
    <template v-else>
      <section class="balance-card">
        ...
        <p class="balance-tip">完成每日任务赚金币，宠物偶尔也会送给你哦</p>
      </section>

      <section class="shop-card">
        <h2>每日任务</h2>
        <div v-if="!missions.length" class="empty-tip">今日暂无任务</div>
        <div v-for="m in missions" :key="m.key" class="mission-row">
          ...
          <div class="badge-info">
            ...
            <p class="badge-desc">{{ m.desc }}（{{ m.progress }}/{{ m.target }}）</p>
          </div>
          <span v-if="m.claimed" class="owned-tag">已领取</span>
          <button
            v-else
            type="button"
            class="buy-btn"
            :class="{ 'claim-now': m.done }"
            :disabled="claimingKey === m.key"
            @click="m.done ? claim(m) : goToMissionPage(m.key)"
          >
            {{ claimingKey === m.key ? '领取中…' : m.done ? '领取' : '去完成' }}
          </button>
        </div>
      </section>
