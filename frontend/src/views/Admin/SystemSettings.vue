...
// ============ 桌宠设置 ============
const petGravity = ref(0.5)
const petGravitySaving = ref(false)
...
function onGravityInput(e: Event) {
  const val = Number((e.target as HTMLInputElement).value)
  petGravity.value = clampGravity(val)
}

async function savePetGravity() {
  petGravitySaving.value = true
  try {
    await adminUpdateSettings({ pet_gravity: String(clampGravity(petGravity.value)) })
    ElMessage.success('桌宠重力已保存，桌宠下次生效')
  } catch (error) {
    ElMessage.error((error as Error).message)
  } finally {
    petGravitySaving.value = false
  }
}
...
onMounted(async () => {
  ...
  try {
    ...
    const g = Number(list.find((s) => s.key === 'pet_gravity')?.value ?? '0.5')
    if (!Number.isNaN(g)) petGravity.value = clampGravity(g)
...
<template>
  <div class="p-4">
    <div v-loading="loading" class="space-y-4">
      ...
      <!-- ====== 桌宠设置 ====== -->
      <div class="rounded border border-ly-line bg-white p-4">
        <h3 class="m-0 mb-1 text-base font-bold">桌宠设置</h3>
        <p class="mt-1 mb-3 text-sm text-slate-500">
          调整桌宠的重力系数（0-1）：越大下落越快，越小越轻飘；0 为无重力悬浮。保存后桌宠重新落点即生效。
        </p>
        <div class="flex items-center gap-3" style="max-width: 420px">
          ...
          />
          <span class="text-sm font-mono text-slate-600 min-w-[40px] text-right">{{ petGravity.toFixed(2) }}</span>
        </div>
        <div class="mt-3">
          <el-button type="primary" :loading="petGravitySaving" @click="savePetGravity">保存</el-button>
        </div>
      </div>
    </div>
  </div>
