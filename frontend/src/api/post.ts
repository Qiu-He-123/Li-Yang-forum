...
export type PostView = 'all' | 'school' | 'hot' | 'latest' | 'today' | 'following'
...
/** 求助帖：楼主标记已解决/未解决 */
export function markPostSolved(postId: number, solved: boolean) {
  return http.post<unknown, { data: { code: number; msg: string; data: Post } }>(
    `/posts/${postId}/solved`,
    { solved },
  )
...
/** 求助帖：楼主选定最佳回复 */
export function setBestComment(postId: number, commentId: number) {
  return http.post<unknown, { data: { code: number; msg: string; data: Post } }>(
    `/posts/${postId}/best-comment`,
    { comment_id: commentId },
  )
}

/** 求助帖：楼主取消最佳回复 */
export function cancelBestComment(postId: number) {
  return http.delete<unknown, { data: { code: number; msg: string; data: Post } }>(
    `/posts/${postId}/best-comment`,
  )
}
