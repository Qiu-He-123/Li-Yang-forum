...
export interface AdminPetCategory {
  id: number
  name: string
  icon: string | null
  sort_order: number
...
export interface AdminPetProduct {
  ...
  /** 仅宠物：陪我玩「动作→宠物说话」JSON 常量 */
  game_speech?: string | null
  /** 仅宠物：是否可飞行（自动触发上抛悬空再落回） */
  can_fly?: boolean
  /** 仅宠物：好感加成（kind=2 食物） */
  affinity_gain?: number
  /** 道具按子类型定制的数值：affinity/satiety/mood/play/stamina/health */
  attrs?: Record<string, number>
}

export interface PetProductCreatePayload {
  name: string
  category: string
  price: number
  original_price?: number | null
...
export function adminPetShopCategories() {
  return http.get<unknown, { data: { code: number; msg: string; data: AdminPetCategory[] } }>(
    '/admin/pet-shop/categories',
  )
}

export function adminPetShopList(
  params: { keyword?: string; category?: string; status?: number; page?: number; page_size?: number } = {},
) {
  return http.get<unknown, { data: { code: number; msg: string; data: PageResp<AdminPetProduct> } }>(
    '/admin/pet-shop/products',
    { params },
  )
}

export function adminPetShopCreate(payload: PetProductCreatePayload) {
  return http.post<unknown, { data: { code: number; msg: string; data: AdminPetProduct } }>(
    '/admin/pet-shop/products',
    payload,
  )
}

export function adminPetShopUpdate(productId: number, payload: Partial<PetProductCreatePayload>) {
  return http.patch<unknown, { data: { code: number; msg: string; data: AdminPetProduct } }>(
    `/admin/pet-shop/products/${productId}`,
    payload,
  )
}

export function adminPetShopToggle(productId: number, status: number) {
  return http.patch<unknown, { data: { code: number; msg: string; data: AdminPetProduct } }>(
    `/admin/pet-shop/products/${productId}/toggle`,
    { status },
  )
}

export function adminPetShopDelete(productId: number) {
  return http.delete(`/admin/pet-shop/products/${productId}`)
}

/** 上传商品图片（≤5MB），返回 { url, size } */
export function adminPetShopUploadImage(file: File) {
  const formData = new FormData()
  formData.append('file', file)
  return http.post<unknown, { data: { code: number; msg: string; data: { url: string; size: number } } }>(
    '/admin/pet-shop/upload-image',
    formData,
    { timeout: 120_000 },
  )
}

/** 上传 3D 模型（GLB/GLTF，≤20MB），返回 { url, size } */
export function adminPetShopUploadModel(
  file: File,
...
// ============ 宠物 AI 设定（后台宠物商城 → 萌宠领养） ============

/** 更新单只宠物的 AI 设定（配合宠物商城管理弹窗「放一起」） */
export function adminPetAiUpdateProduct(
  productId: number,
  payload: { ai_enabled?: boolean; ai_wake_enabled?: boolean; ai_persona?: string; game_speech?: string },
) {
  return http.patch<unknown, { data: { code: number; msg: string; data: any } }>(
    `/admin/pet-ai/products/${productId}/ai`,
    payload,
  )
}
