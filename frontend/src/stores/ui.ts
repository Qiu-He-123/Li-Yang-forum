 ...
 *
 * 启动弹窗编排（announcement → guess → onboarding）：
 * - announcementPopupOpen：公告弹窗显隐；announcementPopupDone：本批次公告已全部处理完（我知道了 / 稍后再说）
 * - guessPopupOpen：今日竞猜押注弹窗显隐；guessPopupDone：今天的押注流程已处理（已押/已跳/手动关/今天无需弹）
 * - onboardingPopupOpen：新手教程显隐
 * - startupPopupsReady：登录 + 基础数据就绪后由 App.vue 置 true，启动编排
 */
export const useUIStore = defineStore('ui', () => {
  ...
  const announcementPopupOpen = ref(false)
  const announcementPopupDone = ref(false)
  const guessPopupOpen = ref(false)
  const guessPopupDone = ref(false)
  const onboardingPopupOpen = ref(false)
  const startupPopupsReady = ref(false)
