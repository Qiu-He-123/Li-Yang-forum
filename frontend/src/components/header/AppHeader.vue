...
const navItems = computed(() => [
  { key: 'home', label: '同伴圈·首页', to: '/', icon: 'home' },
  { key: 'discover', label: '同伴圈·圈子', to: '/circles', icon: 'map-pin' },
...
const pageTitle = computed(() => {
  const path = route.path
  if (path === '/') return '同伴圈·首页'
  if (path === '/post/create') return '发帖'
  if (path === '/notifications') return '同伴圈·消息'
  if (path.startsWith('/user/')) return '同伴圈·我的'
  if (path.startsWith('/circle/')) return '同伴圈·圈子'
  ...
  <header class="site-header">
    <div class="header-inner">
      ...
      <div class="header-side header-side--left" @click="router.push('/')">
        ...
        <span class="brand-text hidden sm:inline">同伴圈</span>
      </div>
