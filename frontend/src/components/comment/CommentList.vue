...
import { setBestComment, cancelBestComment } from '../../api/post'
...
/** 当前用户是否为求助帖楼主（可设置最佳回复） */
const canManageBest = computed(() => {
  if (!props.isHelpPost || props.postAuthorId == null) return false
  return session.userId != null && Number(props.postAuthorId) === Number(session.userId)
})
...
/** 求助帖楼主：设为最佳回复（自动标记帖子已解决） */
async function onSetBest(commentId: number) {
  if (!canManageBest.value) return
  try {
    await setBestComment(props.postId, commentId)
    toast.success('已设为最佳回复')
    await load()
  } catch (error) {
    toast.error((error as Error).message)
  }
...
/** 求助帖楼主：取消最佳回复 */
async function onCancelBest() {
  if (!canManageBest.value) return
  try {
    await cancelBestComment(props.postId)
    toast.success('已取消最佳回复')
    await load()
  } catch (error) {
    toast.error((error as Error).message)
  }
    ...
    <div class="comments-body" :class="{ 'is-guest': !session.userId }">
      ...
      <div v-for="group in grouped" :key="group.root.id" class="comment-group">
        <CommentItem
          ...
          :can-set-best="canManageBest"
          @reply="startReply"
          @deleted="onDeleted"
          @count-updated="onCountUpdated"
          @set-best="onSetBest"
          @cancel-best="onCancelBest"
        />
        <CommentItem
