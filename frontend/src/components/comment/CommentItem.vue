...
const emit = defineEmits<{
  ...
  (e: 'set-best', commentId: number): void
  (e: 'cancel-best', commentId: number): void
}>()
...
function onSetBest() {
  emit('set-best', props.comment.id)
}

function onCancelBest() {
  emit('cancel-best', props.comment.id)
}
    ...
    <div class="comment-head">
      ...
      <div class="comment-meta">
        <div class="comment-name-row">
          ...
          <span v-if="comment.is_best" class="best-badge" title="楼主选定的最佳回复">
      ...
      <div class="comment-actions">
        <button
          v-if="canSetBest"
          class="action-btn action-btn--best"
          :class="{ 'is-best': comment.is_best }"
          type="button"
          :title="comment.is_best ? '取消最佳回复' : '设为最佳回复'"
          @click="comment.is_best ? onCancelBest() : onSetBest()"
        >
          <Icon :name="comment.is_best ? 'trophy' : 'award'" :size="13" />
          <span>{{ comment.is_best ? '取消最佳' : '设为最佳' }}</span>
        </button>
        <button
          class="action-btn"
...
.action-btn--best.is-best {
  color: #ff7a00;
  font-weight: 700;
}
.action-btn--best:hover:not(:disabled) {
