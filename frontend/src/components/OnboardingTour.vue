...
const { onboardingPopupOpen } = storeToRefs(uiStore)
...
// 是否已完成"需要展示新手教程"的前置校验（onboarding_done=false、微信绑定状态已加载、steps 已组装）
...
/**
 * 预检查：拉 onboarding_status。若 true 表示用户已学完，直接告诉编排器"我也 done"
 * （这样编排器不会等它）。若 false，则把步骤 build 好，等待编排器打开时再展示。
 *
 * 注意：**不再在 onMounted / watch(userId) 里直接 visible=true**，
 * 必须等编排器把 uiStore.onboardingPopupOpen 置 true（即公告+押注都处理完）再开。
 */
async function preflight() {
  ...
  try {
    ...
    if (data.onboarding_done) {
      // 已学完：编排器看到 open=true 时也不会展示；这里顺手确保 store 不卡住
      preflightDone = true
      uiStore.onboardingPopupOpen = false
      return
    }
    // 需要展示：先拉微信绑定状态 & 组装 STEPS，等着编排器开
    try {
...
/**
 * 真正开始展示新手教程（由编排器置 uiStore.onboardingPopupOpen=true 时触发）。
 */
async function startIfNeeded() {
  if (!session.userId) return
  if (!uiStore.onboardingPopupOpen) return
  if (!preflightDone) {
    await preflight()
  }
  // 再确认一次：preflight 后还是没打开，说明"已学完 / 无需学"
  if (!uiStore.onboardingPopupOpen) {
    visible.value = false
    return
  }
  stepIdx.value = 0
...
watch(() => session.userId, async (id) => {
  ...
  // 登录后预检查一次：若需要展示，只做 STEPS 准备、**不直接开**。
  // 真正开始时机由编排器驱动（监听 uiStore.onboardingPopupOpen）
  await preflight()
})

/**
 * 编排器驱动：uiStore.onboardingPopupOpen 变 true → 开始展示
 * （确保公告 → 押注 → 新手 的顺序）
 */
watch(
  () => uiStore.onboardingPopupOpen,
  async (shouldOpen) => {
    if (shouldOpen) await startIfNeeded()
    else {
      visible.value = false
      document.body.classList.remove('tour-body-lock')
      setScrollLock(false)
...
onMounted(() => {
  // 首次挂载时：如果用户已登录，把 STEPS 先组装好（还是不开，等编排器）
  if (session.userId) void preflight()
