...
const props = defineProps<{
  ...
  /** 求助帖：初始悬赏金币 */
  initialRewardCoins?: number | null
}>()
...
/** 求助帖：悬赏金币（仅求助区显示并必填） */
const rewardCoins = ref<number | ''>(props.initialRewardCoins ?? '')
/** 是否求助区（显示悬赏金币输入框） */
const isHelpCategory = computed(() => {
  const c = circles.value.find((x) => x.slug === category.value || x.name === category.value)
  const slug = c?.slug
  return category.value === 'help' || category.value === '求助区' || slug === 'help'
})
...
watch(
  ...
  () => {
    ...
    pollData.value = null
    rewardCoins.value = props.initialRewardCoins ?? ''
  },
)
...
function buildPayload(asDraft: boolean) {
  return {
    ...
    poll: pollData.value,
    // 求助帖：悬赏金币（仅求助区填写）
    reward_coins: isHelpCategory.value ? (rewardCoins.value === '' ? null : Number(rewardCoins.value)) : null,
  }
}
...
async function publish(asDraft = false) {
  ...
  // 求助帖：求助区必须填写悬赏金币（正整数）
  if (isHelpCategory.value) {
    const rc = Number(rewardCoins.value)
    if (rewardCoins.value === '' || !Number.isFinite(rc) || rc <= 0) {
      toast.error('求助帖必须填写悬赏金币（大于0）')
      return
    }
  }
  // 非草稿发布时，如果未选择圈子，弹出提示
...
<template>
  <div class="editor-wrap">
    ...
    <section class="editor-card">
      ...
      <!-- 求助帖：悬赏金币（仅求助区显示并必填） -->
      <div v-if="isHelpCategory" class="reward-row">
        <span class="reward-ic">
          <Icon name="coin" :size="18" color="#ffb300" />
        </span>
...
.reward-ic {
  ...
  height: 30px;
  border-radius: 10px;
  background: linear-gradient(135deg, #fff3d6, #ffe9b3);
  flex-shrink: 0;
}
.reward-field {
  flex: 1;
