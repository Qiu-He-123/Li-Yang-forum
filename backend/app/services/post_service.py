...
def _is_help_category(db: Session, category: str) -> bool:
    """判断分类是否求助区（接受 slug 'help' 或名称 '求助区'）。"""
    if category in ("help", "求助区"):
        return True
    return False
...
def post_dict(
    ...
    data = {
        ...
        # 求助帖扩展字段
        "reward_coins": post.reward_coins,
        "is_solved": bool(post.is_solved),
        "best_comment_id": post.best_comment_id,
        "solved_at": to_iso_zh(post.solved_at),
...
def list_posts(
    ...
    # ========== 关注流 view=following：需登录；过滤 author_id ∈ 已关注 + 默认好友 ==========
    if view == "following":
        if user is None:
            # 未登录：返回空结果（不抛 401，保证广场 tab 切换不中断）
        ...
        followed_ids = {
            row[0] for row in db.execute(
                sa_select(Follow.followee_id).where(Follow.follower_id == user.id)
            ).all()
        }
        # 默认好友（官方号）自动进入关注流
        try:
            ...
            default_ids = set()
        allowed_ids = followed_ids | default_ids
        # 关注流里"我"自己的帖子也可以看到（方便自己回看）
        allowed_ids.add(user.id)
        query = query.where(Post.author_id.in_(list(allowed_ids)))
    # 今日发布：登录/匿名都按当天过滤
    if view == "today":
...
async def create_post(payload: PostCreate, request: Request, db: Session, user: User) -> dict:
    ...
    # 求助帖：求助区必须填写悬赏金币（草稿也需填写，正式发布前校验）
    if _is_help_category(db, payload.category):
        if not payload.reward_coins or payload.reward_coins <= 0:
            raise HTTPException(status_code=400, detail=ErrorCode.HELP_REWARD_REQUIRED)
    ...
    post = Post(
        ...
        # 求助帖悬赏金币（仅求助区填写）
        reward_coins=payload.reward_coins if _is_help_category(db, payload.category) else None,
    )
    db.add(post)
    db.flush()
...
async def update_post(post_id: int, payload: PostUpdate, request: Request, db: Session, user: User) -> dict:
    ...
    if "category" in changes and not _is_valid_category(db, changes["category"]):
        raise HTTPException(status_code=400, detail=ErrorCode.CATEGORY_NOT_FOUND)
    # 求助帖：求助区必须保留悬赏金币
    target_is_help = _is_help_category(db, changes.get("category", post.category))
    if target_is_help:
        new_reward = changes.get("reward_coins", post.reward_coins)
        if not new_reward or new_reward <= 0:
            raise HTTPException(status_code=400, detail=ErrorCode.HELP_REWARD_REQUIRED)
...
def mark_post_solved(post_id: int, solved: bool, db: Session, user: User) -> dict:
    """求助帖楼主标记已解决/未解决（仅求助帖且仅楼主）。"""
    post = db.get(Post, post_id)
    if not post:
        raise HTTPException(status_code=404, detail=ErrorCode.POST_NOT_FOUND)
    if post.author_id != user.id:
        raise HTTPException(status_code=403, detail=ErrorCode.NO_PERMISSION)
    if not _is_help_category(db, post.category):
        raise HTTPException(status_code=400, detail=ErrorCode.HELP_REWARD_REQUIRED)
    post.is_solved = bool(solved)
    post.solved_at = datetime.utcnow() if solved else None
    db.commit()
    db.refresh(post)
    return post_dict(post, db=db)


def set_best_comment(post_id: int, comment_id: int, db: Session, user: User) -> dict:
    """求助帖楼主选定最佳回复（同时自动标记已解决）。"""
    post = db.get(Post, post_id)
    if not post:
        raise HTTPException(status_code=404, detail=ErrorCode.POST_NOT_FOUND)
    if post.author_id != user.id:
    ...
    if not comment or comment.post_id != post_id:
        raise HTTPException(status_code=400, detail=ErrorCode.COMMENT_NOT_BEST)
    # 清除旧最佳回复标记
    if post.best_comment_id and post.best_comment_id != comment_id:
        old = db.get(Comment, post.best_comment_id)
        if old:
            old.is_best = False
            old.best_at = None
    comment.is_best = True
    comment.best_at = datetime.utcnow()
...
def cancel_best_comment(post_id: int, db: Session, user: User) -> dict:
    """求助帖楼主取消最佳回复（不改变解决状态）。"""
    post = db.get(Post, post_id)
    if not post:
        raise HTTPException(status_code=404, detail=ErrorCode.POST_NOT_FOUND)
