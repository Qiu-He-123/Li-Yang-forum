...
def _issue_tokens(response: Response, db: Session, user: User) -> dict[str, str]:
    ...
    response.set_cookie("access_token", access, httponly=True, samesite="lax", max_age=access_max_age, path="/")
    response.set_cookie("refresh_token", refresh, httponly=True, samesite="lax", max_age=refresh_max_age, path="/")
...
def refresh_session(refresh_token: str | None, request: Request, response: Response, db: Session) -> dict[str, Any]:
    ...
    access_max_age = settings.access_token_expire_minutes * 60
    refresh_max_age = settings.refresh_token_expire_days * 86400
    response.set_cookie("access_token", new_access, httponly=True, samesite="lax", max_age=access_max_age, path="/")
    response.set_cookie("refresh_token", new_refresh, httponly=True, samesite="lax", max_age=refresh_max_age, path="/")
