...
def _gathering_item(g: Gathering, host: User | None) -> dict:
    return {
        "id": g.id,
        "title": g.title,
        "type": g.type,
        ...
        "host_id": g.host_id,
        "author": host.nickname if host else None,
        "author_avatar_url": host.avatar_url if host else None,
        "created_at": to_iso_zh(g.created_at) if g.created_at else None,
    }


def admin_gatherings(db: Session, page: int = 1, page_size: int = 20, keyword: str | None = None, status: str | None = None) -> dict:
    """组局列表（管理端，含已取消/已结束，分页 + 标题/描述搜索 + 状态过滤）。"""
    query = select(Gathering).order_by(desc(Gathering.created_at))
    if keyword:
        kw = f"%{keyword}%"
        query = query.where(Gathering.title.like(kw) | Gathering.description.like(kw))
    if status:
        query = query.where(Gathering.status == status)
    total = db.scalar(select(func.count()).select_from(query.subquery())) or 0
    rows = db.scalars(query.offset((page - 1) * page_size).limit(page_size)).all()
    host_map: dict[int, User] = {}
    if rows:
        host_ids = {g.host_id for g in rows}
        host_map = {u.id: u for u in db.scalars(select(User).where(User.id.in_(host_ids))).all()}
    return {
        "items": [_gathering_item(g, host_map.get(g.host_id)) for g in rows],
        "total": int(total),
        "page": page,
        "page_size": page_size,
    }
...
def admin_delete_gathering(gathering_id: int, reason: str, request: Request, db: Session, admin: Admin) -> None:
    """管理员删除组局：需理由，通知发起人及全部报名者，并清理报名记录。"""
    ...
    if g.host_id:
        db.add(Notification(
            user_id=g.host_id, type="system",
            title="组局已被删除",
            content=f"你的组局「{g.title}」已被管理员删除。删除理由：{reason}",
            reference_type="gathering", reference_id=gathering_id,
        ))
    # 通知其他报名者
    participant_ids = db.scalars(
        select(GatheringParticipant.user_id).where(GatheringParticipant.gathering_id == gathering_id)
    ).all()
    for uid in participant_ids:
        ...
        db.add(Notification(
            ...
            title="组局已被删除",
            content=f"你报名的组局「{g.title}」已被管理员删除。删除理由：{reason}",
            reference_type="gathering", reference_id=gathering_id,
        ))
    # 删除报名记录 + 组局本体
    db.execute(delete(GatheringParticipant).where(GatheringParticipant.gathering_id == gathering_id))
    db.delete(g)
    log_admin_action(
        db, admin.id, "delete_gathering",
        json.dumps({"gathering_id": gathering_id, "title": g.title, "reason": reason}, ensure_ascii=False),
        _extract_ip(request),
    )
    db.commit()
