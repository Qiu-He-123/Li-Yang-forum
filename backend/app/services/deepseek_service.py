...
11. 灌水无意义：标题或正文无实际信息量（如标题仅"12"、内容仅"......"、
    纯标点符号、重复刷屏、凑字数），属于低质量内容，拦截但按轻微违规处理
...
- 标题和正文都要审核：标题违规同样拦截，标题/正文无实际信息量按灌水处理
...
输入: "标题：12\n内容：......"
输出: {"pass": false, "reason": "标题和内容均为无实际信息量的灌水内容", "category": "灌水水帖", "severity": "low"}

输入: "标题：今天心情不错\n内容：哈哈哈哈哈"
输出: {"pass": false, "reason": "内容缺乏实际信息量，属于灌水", "category": "灌水水帖", "severity": "low"}
"""
...
def audit_content(db: Session, content: str) -> dict[str, Any]:
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": f"请审核以下内容（含标题与正文），只返回 JSON：\n{content[:2000]}"},
        ],
        "temperature": 0.1,
        "max_tokens": 300,
    }
