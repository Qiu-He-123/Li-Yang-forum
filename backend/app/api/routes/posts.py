...
@router.get("")
def list_posts(
    view: str = Query(default="all", pattern="^(all|school|hot|latest|today|following)$"),
    ...
    view=following：关注流，需要登录；未登录用户访问 following 时返回空列表（不抛 401，避免中断广场切换）。
    """
...
def mark_solved(
    ...
    """求助帖楼主标记已解决/未解决。body: {"solved": true|false}"""
    solved = bool(payload.get("solved", True))
    return ok(post_service.mark_post_solved(post_id, solved, db, user))


@router.post("/{post_id}/best-comment")
def set_best_comment(
    ...
    """求助帖楼主选定最佳回复。body: {"comment_id": 123}"""
    comment_id = payload.get("comment_id")
    if not isinstance(comment_id, int) or comment_id <= 0:
        raise HTTPException(status_code=400, detail=ErrorCode.COMMENT_NOT_BEST)
    return ok(post_service.set_best_comment(post_id, comment_id, db, user))


@router.delete("/{post_id}/best-comment")
def cancel_best_comment(
    post_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(current_user),
) -> dict:
    """求助帖楼主取消最佳回复。"""
    return ok(post_service.cancel_best_comment(post_id, db, user))
