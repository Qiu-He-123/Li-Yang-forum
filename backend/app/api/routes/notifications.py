...
from app.services import message_service, notification_service, pet_ai_service
...
def unread_count(db: Session = Depends(get_db), user: User = Depends(current_user_allow_banned)) -> dict:
    ...
    result["dm_unread"] = (
        message_service.count_unread_messages(user.id, db)
        + pet_ai_service.count_unread_pet_ai_messages(db, user.id)
    )
    return ok(result)
