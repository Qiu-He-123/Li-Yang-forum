...
# ===== DEV-ONLY：E2E 测试临时会话端点（测试后必须删除）=====
from fastapi import HTTPException  # noqa: E402
from sqlalchemy import select  # noqa: E402

@router.post("/dev-session")
def dev_session(username: str = "testuser01", db: Session = Depends(get_db), response: Response = None) -> dict:
    """【仅开发】按用户名免密登录，用于浏览器 E2E 测试绕过图形验证码。禁止上生产。"""
    user = db.scalars(select(User).where(User.username == username)).first()
    if not user:
        raise HTTPException(status_code=404, detail="user not found")
    auth_service._issue_tokens(response, db, user)
    return ok({"user_id": user.id, "nickname": user.nickname})
